...
- 4c43df7: Add `--token` (`-t`) option to all `group` and `group member` subcommands
...
- 70972fa: fix: skip HTML comment obfuscation check inside code blocks
...
- 074d1e0: Add group management CLI and group-path publish support.
...
- 29a87ae: Allow version specifier for local source_type skill install.
...
- 2a888ed: Add 8 diagnostic checks to `doctor` command
...
- 9975a1b: Auto-detect multi-skill parent directories and install all child skills
...
- b94b47d: Add token authentication to install command
...
- 9fda622: Add interactive token prompt for `login` command
...
- c7d274f: Fix self-hosted GitLab/GitHub clone URL for web-published skills
...
- e7a61aa: Fix multi-skill repo installation path using skill_path
...
- fd1404c: Add Agents column to `list` command and fix global path
...
- 3f41e5b: Support `metadata.alwaysApply` in SKILL.md frontmatter for Cursor bridge rules
...
- 2848258: Add `metadata.globs` support for Cursor bridge rules
...
- 8c1f376: Support rush-app private registry with `/api/reskill` API prefix
...
- e870c4c: Use login-cli endpoint for CLI authentication
